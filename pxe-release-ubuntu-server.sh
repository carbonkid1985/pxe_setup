...
cat >> "${distro_menu_path}" << EOF
LABEL ${rand}
	MENU LABEL Ubuntu ${version} Server ${arch} ${menu_flavour}
        KERNEL /ubuntu/server/${version}/${arch}/${de}/linux
        INITRD /ubuntu/server/${version}/${arch}/${de}/initrd.gz
	APPEND text vga=788 locale=en_GB.UTF-8 keyboard-configuration/layout=gb
        TEXT HELP
        Boot Ubuntu ${version} Server ${arch} ${menu_flavour}
ENDTEXT
...
LABEL ${rand}
        MENU LABEL Ubuntu ${version} Server ${arch} ${menu_flavour}
        KERNEL /ubuntu/server/${version}/${arch}/${de}/casper/vmlinuz
        INITRD /ubuntu/server/${version}/${arch}/${de}/casper/initrd
        APPEND ip=dhcp boot=casper text vga=normal netboot=nfs nfsroot=${nfs_server_ip}:${distro_dir}/${version}/${arch}/${de} locale=en_GB.UTF-8 keyboard-configuration/layout=gb splash
        TEXT HELP
	Boot Ubuntu ${version} Server ${arch} ${menu_flavour}
ENDTEXT
